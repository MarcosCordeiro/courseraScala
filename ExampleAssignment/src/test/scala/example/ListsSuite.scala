 ...
 */
 @RunWith(classOf[JUnitRunner])
  ...
  test("throws an exception if its list have 0 arguments") {
    intercept[NoSuchElementException] {
      max(List())
    }
  }

  test("max of same values arguments") {
    assert(max(List(3, 3, 3)) === 3)
  }

  test("max of value zero") {
    assert(max(List(0)) === 0)
  }

  test("max of negative values") {
    assert(max(List(-3, -2, -1)) === 0)
  }
}
